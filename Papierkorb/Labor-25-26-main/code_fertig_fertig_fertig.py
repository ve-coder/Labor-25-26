...
STEP_DELAY = 0.001  # 5ms zwischen jedem Schritt = langsame, sanfte Bewegung
...
def get_key():
    """Read a single keypress - works in more environments"""
    try:
        # Try raw mode first (for proper terminals)
        import tty
        import termios
        ...
        # Fallback:  use regular input (requires Enter key)
        return input().strip().lower()[:1] if input else ''
...
def main():
    ...
    if sys.stdin. isatty():
        print("\n✓ Terminal erkannt - Einzeltasten-Modus aktiv")
        print("⚠️  Motor ist AUSGESCHALTET.  Drücke 'r' oder 'l' zum Bewegen.\n")
    else:
        print("\n⚠️  Kein Terminal erkannt - Eingabe mit ENTER bestätigen")
        print("⚠️  Motor ist AUSGESCHALTET. Tippe 'r' oder 'l' und drücke ENTER.\n")
    ...
    try:
        while True:
            try:
                ...
                print(f"\n❌ Fehler beim Lesen der Eingabe: {e}")
                print("Versuche normale Eingabe mit ENTER...")
                key = input("Eingabe (r/l/o/q): ").strip().lower()[:1]
